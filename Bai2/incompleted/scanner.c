...
// bo qua chu thich nhung phai tim thay ')' khong tim thay thi error neu 
void skipComment() {
...
// gap ki tu thi doc 1 chuoi gom ki tu va so
// No co the la truong hop keyword
// Can kiem tra xem co vuot qua kich thuoc mang khong
Token* readIdentKeyword(void) {
  ...
  while (currentChar!=EOF && (charCodes[currentChar]==CHAR_LETTER || charCodes[currentChar]==CHAR_DIGIT))
  {
      ...
      {
      // strcat(token->string,(char)currentChar);
  ...
  if (index>MAX_IDENT_LEN)
  {
    error(ERR_IDENTTOOLONG,token->lineNo,colNo);
  }else if(checkKeyword(token->string) != TK_NONE){
